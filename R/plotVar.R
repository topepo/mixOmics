  ...
  function(object,
    ...
    if (style == "3d") {
      if (any(class.object %in% c(object.pls, object.blocks))) {
        if (any(class.object == "rcc")) {
          ...
          cord.X[[1]] <- cor(object$X, object$variates$X[, c(comp1, comp2, comp3)], use = "pairwise")
          cord.X[[2]] <- cor(object$Y, if (object$mode == "canonical") {
          ...
          cord.X[[1]] <- cor(
            object$X[, colnames(object$X) %in% unique(unlist(lapply(unique(c(comp1, comp2, comp3, comp.select)), function(x) {
              selectVar(object, comp = x)$name
            })))], # variables selected at least once on unique(comp1, comp2, comp3 and comp.select
            object$variates$X[, c(comp1, comp2, comp3, comp.select)],
            use = "pairwise"
          )
          ind.var.sel[[1]] <- sample.X[[1]] <- seq_len(length(colnames(object$X)))
          if (!is.null(comp.select)) {
            cord.X[[1]] <- cord.X[[1]][row.names(cord.X[[1]]) %in% unique(unlist(lapply(comp.select, function(x) {
              selectVar(object, comp = x)$name
            }))), , drop = FALSE]
          }
          ...
          cord.X[[1]] <- cor(
            object$X[, colnames(object$X) %in% unique(unlist(lapply(c(comp1, comp2, comp3), function(x) {
          ...
          cord.X[[2]] <- cor(
            object$Y[, colnames(object$Y) %in% unique(unlist(lapply(c(comp1, comp2, comp3), function(x) {
          ...
          if (!is.null(comp.select)) {
            ...
            cord.X[[2]] <- cord.X[[2]][row.names(cord.X[[2]]) %in% unique(unlist(lapply(comp.select, function(x) {
              selectVar(object, comp = x)$Y$name
            }))), , drop = FALSE]
          }
          ind.var.sel[[1]] <- which(colnames(object$X) %in% rownames(cord.X[[1]]))
          ...
          cord.X <- lapply(blocks, function(x) {
            cor(object$blocks[[x]], object$variates[[x]][, c(comp1, comp2, comp3)], use = "pairwise")
          })
          ind.var.sel <- sample.X <- lapply(object$blocks, function(x) {
            seq_len(ncol(x))
          })
          if (!is.null(comp.select)) {
            cord.X <- lapply(seq_len(length(cord.X)), function(z) {
              cord.X[[z]][row.names(cord.X[[z]]) %in% unique(unlist(lapply(comp.select, function(x) {
      ...
      if (any(class.object %in% c(object.pls, object.blocks))) {
        if (any(class.object == "rcc")) {
          ...
          cord.X[[1]] <- cor(
            object$X[, colnames(object$X) %in% unique(unlist(lapply(comp.select, function(x) {
              selectVar(object, comp = x)$name
            }))), drop = FALSE],
            object$variates$X[, unique(c(comp1, comp2))],
            use = "pairwise"
          )
          ind.var.sel[[1]] <- sample.X[[1]] <- seq_len(length(colnames(object$X)))
          # if (!is.null(comp.select)) {
          ...
          cord.X[[2]] <- cor(
            object$Y[, colnames(object$Y) %in% unique(unlist(lapply(comp.select, function(x) {
              selectVar(object, comp = x)$Y$name
            }))), drop = FALSE],
            if (object$mode == "canonical") {
              object$variates$Y[, c(comp1, comp2)]
          ...
          cord.X <- lapply(blocks, function(x) {
            cor(object$blocks[[x]], object$variates[[x]][, c(comp1, comp2)], use = "pairwise")
          })
          ind.var.sel <- sample.X <- lapply(object$blocks, function(x) {
            seq_len(ncol(x))
          })
          if (!is.null(comp.select)) {
            cord.X <- lapply(seq_len(length(cord.X)), function(z) {
              cord.X[[z]][row.names(cord.X[[z]]) %in% unique(unlist(lapply(comp.select, function(x) {
                selectVar(object, block = blocks[z], comp = x)[[1]]$name
              }))), , drop = FALSE]
            })
          }
          for (i in seq_len(length(cord.X)))
    ...
    if (any(class.object %in% object.blocks)) {
      VarX <- lapply(seq_len(2), function(j) {
        do.call(cbind, lapply(object$variates, function(i) i[, comp[j]]))
      })
    ...
    if (missing(pch)) {
      ...
      if (style == "3d") {
        pch <- unlist(lapply(seq_len(length(cord.X)), function(x) {
          rep(c("sphere", "tetra", "cube", "octa", "icosa", "dodeca")[x], sum(sapply(cord.X[x], nrow)))
        }))
      } else {
        pch <- unlist(lapply(seq_len(length(cord.X)), function(x) {
    ...
    if (is.null(var.names)) {
      ...
      var.names.list <- unlist(lapply(seq_len(length(var.names)), function(x) {
        if (var.names[x]) {
          rownames(cord.X[[x]])
        } else {
      ...
      length.var.names <- sum(sapply(seq_len(length(var.names)), function(x) {
      ...
      var.names.list <- unlist(sapply(seq_len(length(var.names)), function(x) {
        if (lapply(var.names, is.logical)[[x]]) {
          if (var.names[[x]]) {
            row.names(cord.X[[x]])
          } else {
    ...
    #-- Start: data set
    df <- data.frame(do.call(rbind, cord.X), "Block" = paste0("Block: ", unlist(lapply(seq_len(length(cord.X)), function(z) {
    ...
    if (overlap) {
      df$Overlap <- title
      df$Block <- factor(unlist(lapply(seq_len(length(cord.X)), function(z) {
        rep(blocks[z], nrow(cord.X[[z]]))
      })))
    ...
    if (style == "ggplot2" & plot) {
      ...
      for (i in seq_len(length(var.names))) {
        if (var.names[i]) {
          p <- p + geom_text(
            data = df[c((ind.group[i] + 1):ind.group[i + 1]), ],
    ...
    if (style == "lattice") {
      legend.lattice <- list(
        space = "right", title = legend.title, cex.title = 1.25,
        points = list(col = unique(df$col), cex = unique(df$cex), pch = unique(df$pch)),
        text = list(blocks)
      )

      if (overlap) {
        p <- xyplot(y ~ x | Overlap,
          data = df, xlab = X.label, ylab = Y.label, main = title,
          scales = list(
            x = list(relation = "free", limits = c(-1, 1)),
          ...
          panel = function(x, y, ...) {
            ...
            for (i in seq_len(length(var.names))) {
              if (var.names[i]) {
                panel.text(
                  x = df[c((ind.group[i] + 1):ind.group[i + 1]), "x"],
        ...
        for (i in seq_len(length(c("Main circle", "Inner circle")))) {
          panel.lines(
            x = circle[circle$Circle %in% c("Main circle", "Inner circle")[i], "x"],
        ...
        p <- xyplot(y ~ x | Block,
          data = df, xlab = X.label, ylab = Y.label, main = title, as.table = TRUE,
          scales = list(
            x = list(relation = "free", limits = c(-1, 1)),
            y = list(relation = "free", limits = c(-1, 1))
          ),
        ...
        for (k in seq_len(length(cord.X))) {
          ...
          if (var.names[k]) {
            panel.text(
              x = df[c((ind.group[k] + 1):ind.group[k + 1]), "x"],
    ...
    if (style == "graphics") {
      if (overlap) {
        ...
        for (k in seq_len(length(cord.X))) {
          ...
          if (var.names[k]) {
            text(
              x = df[df$Block %in% paste0("Block: ", blocks[k]), "x"],
    ...
    if (style == "3d") {
      ...
      if (any(axes.box == "axes") || any(axes.box == "all")) {
        rgl::axes3d(c("x", "y", "z"),
          pos = c(0, 0, 0), nticks = 2, at = c(-1.2, 1.2),
          tick = FALSE, labels = ""
        )
      }
      ...
      for (i in seq_len(length(var.names))) {
        if (var.names[i]) {
          ...
          switch(unique(df[c((ind.group[i] + 1):ind.group[i + 1]), "pch"]),
            ...
            tetra = rgl::shapelist3d(rgl::tetrahedron3d(),
              x = df[c((ind.group[i] + 1):ind.group[i + 1]), "x"],
              y = df[c((ind.group[i] + 1):ind.group[i + 1]), "y"],
              z = df[c((ind.group[i] + 1):ind.group[i + 1]), "z"],
              col = df[c((ind.group[i] + 1):ind.group[i + 1]), "col"],
            ...
            cube = rgl::shapelist3d(rgl::cube3d(),
              x = df[c((ind.group[i] + 1):ind.group[i + 1]), "x"],
              y = df[c((ind.group[i] + 1):ind.group[i + 1]), "y"],
              z = df[c((ind.group[i] + 1):ind.group[i + 1]), "z"],
              col = df[c((ind.group[i] + 1):ind.group[i + 1]), "col"],
            ...
            octa = rgl::shapelist3d(rgl::octahedron3d(),
              x = df[c((ind.group[i] + 1):ind.group[i + 1]), "x"],
              y = df[c((ind.group[i] + 1):ind.group[i + 1]), "y"],
              z = df[c((ind.group[i] + 1):ind.group[i + 1]), "z"],
              col = df[c((ind.group[i] + 1):ind.group[i + 1]), "col"],
            ...
            icosa = rgl::shapelist3d(rgl::icosahedron3d(),
              x = df[c((ind.group[i] + 1):ind.group[i + 1]), "x"],
              y = df[c((ind.group[i] + 1):ind.group[i + 1]), "y"],
              z = df[c((ind.group[i] + 1):ind.group[i + 1]), "z"],
              col = df[c((ind.group[i] + 1):ind.group[i + 1]), "col"],
            ...
            dodeca = rgl::shapelist3d(rgl::dodecahedron3d(),
              x = df[c((ind.group[i] + 1):ind.group[i + 1]), "x"],
              y = df[c((ind.group[i] + 1):ind.group[i + 1]), "y"],
              z = df[c((ind.group[i] + 1):ind.group[i + 1]), "z"],
              col = df[c((ind.group[i] + 1):ind.group[i + 1]), "col"],
      ...
      if (any(axes.box == "axes") || any(axes.box == "all")) {
        ...
        Z <- c(
          0.0, 0.035, -0.035, 0.0, 0.035, -0.035, 0.0, 0.0, 0.0, 0.0, 0.035 * sin(pi / 4), -0.035 * sin(pi / 4),
          0.0, 0.035, -0.035, 0.0, 0.0, 0.0, 0.0, 0.035 * sin(pi / 4), -0.035 * sin(pi / 4), 0.0, -0.035 * sin(pi / 4), 0.035 * sin(pi / 4),
          1.2, 1.09, 1.09, 1.2, 1.09, 1.09, 1.2, 1.09, 1.09, 1.2, 1.09, 1.09
        )
      ...
      if (any(axes.box == "bbox") || any(axes.box == "all")) {
        rgl::bbox3d(
          color = c("#333377", "black"), emission = gray(0.5),
