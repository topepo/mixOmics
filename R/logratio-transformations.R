...
logratio.transfo <- function(X,
  ...
  if (logratio == "ILR") {
    if (!is(X, "ilr")) { # data are ilr transformed, then the data lose 1 variable, but we'll use V to reconstruct the matrix
...
ilr.transfo <- function(x, fast = TRUE, offset = 0) {
  ...
  if (fast) {
    ...
    for (i in 1:ncol(x.ilr))
    {
      x.ilr[, i] <- sqrt((D - i) / (D - i + 1)) * log(apply(as.matrix(x[, (i + 1):D]), 1, function(x) {
        exp(log(x))
      }) / (x[, i] + offset) + offset)
