...
test_that("plotIndiv works for rcc", {
  ...
  pl.res <- plotIndiv(nutri.res,
    rep.space = "XY-variate", group = nutrimouse$genotype,
    legend = TRUE
  )
...
test_that("plotIndiv works for (s)pls", {
  ...
  toxicity.spls <- spls(X, Y,
    ncomp = 3, keepX = c(50, 50, 50),
    keepY = c(10, 10, 10)
  )

  pl.res <- plotIndiv(toxicity.spls,
    rep.space = "X-variate", ind.name = FALSE,
...
test_that("plotIndiv works for mint.(s)plsda", {
  data(stemcells)
  res <- mint.splsda(
    X = stemcells$gene, Y = stemcells$celltype, ncomp = 2, keepX = c(10, 5),
...
test_that("plotIndiv works for sgcca and rgcca", {
  ...
  nutrimouse.sgcca <- wrapper.sgcca(
    X = data,
    design = design1,
    penalty = c(0.3, 0.5, 1),
    ncomp = 3,
...
test_that("plotIndiv works for sgccda", {
  ...
  nutrimouse.sgccda1 <- wrapper.sgccda(
    X = data,
    Y = Y,
    design = design1,
    ncomp = 2,
...
test_that("plotIndiv.sgccda(..., blocks = 'average') works with ind.names and ell", code = {
  data("breast.TCGA")
  data <- list(
    mrna = breast.TCGA$data.train$mrna, mirna = breast.TCGA$data.train$mirna,
  ...
  TCGA.block.splsda <- block.splsda(
    X = data, Y = breast.TCGA$data.train$subtype,
