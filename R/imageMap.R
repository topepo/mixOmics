...
imageMap <-
  function(mat,
    ...
    if (!is.null(row.sideColors)) {
      lmat <- cbind(
        lmat[, 1], c(rep(NA, nrow(lmat) - 1), nrow(lmat) + 2),
    ...
    par(usr = c(0, 1, 0, 1))
    lv <- c(
      min.breaks, (3 * min.breaks + max.breaks) / 4,
    ...
    if (isTRUE(csc)) {
      ...
      image(1:nc, 1:n.csc, img,
        col = sideColors, axes = FALSE, xlab = "",
        ylab = ""
      )
      abline(
        h = 1:(n.csc - 1) + 0.5, lwd = 2,
    ...
    if (isTRUE(rsc)) {
      ...
      image(1:n.rsc, 1:nr, img,
        col = sideColors, axes = FALSE, xlab = "",
        ylab = ""
      )
      abline(
        v = 1:(n.rsc - 1) + 0.5, lwd = 2,
        col = ifelse(par("bg") == "transparent", "white", par("bg"))
      )
    }
    ...
    axis(1, 1:nc,
      labels = col.names, las = 2, line = -0.5, tick = 0,
      cex.axis = col.cex
    )
    ...
    axis(4, 1:nr,
      labels = row.names, las = 2, line = -0.5, tick = 0,
      cex.axis = row.cex
    )
      ...
      repeat {
        ...
        if (isTRUE(zone)) {
          ...
          if (isTRUE(csc)) {
            ...
            img <- matrix(c(1:(n.csc * nr.zoom)),
              ncol = n.csc,
              byrow = FALSE
            )

            image(1:nr.zoom, 1:n.csc, img,
              col = sideColors,
          ...
          if (isTRUE(rsc)) {
            ...
            sideColors <- as.vector(r.sideColors)
            img <- matrix(1:(n.rsc * nc.zoom),
              nrow = n.rsc,
              byrow = TRUE
            )

            image(1:n.rsc, 1:nc.zoom, img,
              col = sideColors,
              axes = FALSE, xlab = "", ylab = ""
            )
            abline(
          ...
          # layout 6
          par(mar = c(margins[1], 0, 0, margins[2]))
          image(1:nr.zoom, 1:nc.zoom, mat.zoom,
            col = color,
            breaks = breaks, axes = FALSE, xlab = "", ylab = ""
          )

          axis(1, 1:nr.zoom,
            labels = rlab.zoom, las = 2,
