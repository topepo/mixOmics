...
#' wrap.result.sgcca <- wrapper.sgcca(
#'   X = data, design = design, penalty = c(.3, .5, 1),
#'   ncomp = 2,
#'   scheme = "centroid"
#' )
#' wrap.result.sgcca
...
wrapper.sgcca <-
  function(X,
    ...
    check <- Check.entry.sgcca(
      X = X, design = design, ncomp = ncomp, scheme = scheme, scale = scale,
    ...
    result.sgcca <- internal_mint.block(
      A = A, design = design, tau = NULL,
      ncomp = ncomp,
