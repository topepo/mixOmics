                ...
                if (missing(study)) # block.plsda
                  {
                    ...
                    res <- block.plsda(
                      X = X, Y = Y, indY = indY, ncomp = ncomp, design = design, scheme = scheme,
                  ...
                  res <- mint.block.plsda(
                    X = X, Y = Y, indY = indY, study = study, ncomp = ncomp, design = design, scheme = scheme,
                    scale = scale, init = init, tol = tol, max.iter = max.iter, near.zero.var = near.zero.var
                  )
                }
              } else {
                if (missing(study)) # block.splsda
                  {
                    ...
                    res <- mint.block.splsda(
                      X = X, Y = Y, indY = indY, ncomp = ncomp, keepX = keepX,
                  ...
                  res <- mint.block.splsda(
                    X = X, Y = Y, indY = indY, ncomp = ncomp, study = study, keepX = keepX,
                    design = design, scheme = scheme, scale = scale, init = init, tol = tol,
                    max.iter = max.iter, near.zero.var = near.zero.var
                  )
                }
        ...
        if (missing(keepX)) # RGCCA
          {
            ...
            res <- wrapper.rgcca(
              X = X, design = design, tau = tau, ncomp = ncomp,
          ...
          res <- wrapper.rgcca(
            X = X, design = design, tau = tau, ncomp = ncomp, keepX = keepX,
            max.iter = max.iter, scheme = scheme, scale = scale, init = init, tol = tol
          )
        }
      }
    ...
    if (is.factor(Y)) # either plsda, splsda
      {
        ...
        if (missing(keepX) & missing(keepY)) # plsda, mint.plsda
          ...
          if (missing(study)) {
            ...
            res <- mint.splsda(
              X = X, Y = Y, ncomp = ncomp, study = study, keepX = keepX,
              max.iter = max.iter, tol = tol, near.zero.var = near.zero.var, scale = scale
            )
          }
        }
      ...
      if (missing(keepX) & missing(keepY)) # pls, mint.pls
        {
          if (missing(study)) {
            ...
            res <- mint.pls(
              X = X, Y = Y, ncomp = ncomp, mode = mode, study = study,
              max.iter = max.iter, tol = tol, near.zero.var = near.zero.var, scale = scale
            )
          }
        } else {
        if (missing(study)) {
          ...
          res <- spls(
            X = X, Y = Y, ncomp = ncomp, mode = mode, keepX = keepX, keepY = keepY,
          ...
          res <- mint.spls(
            X = X, Y = Y, ncomp = ncomp, mode = mode, study = study, keepX = keepX, keepY = keepY,
            max.iter = max.iter, tol = tol,
            near.zero.var = near.zero.var, scale = scale
          )
        }
