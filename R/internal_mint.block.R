...
internal_mint.block <-
  function(A, indY = NULL, design = 1 - diag(length(A)),
    ...
    A <- lapply(mean_centered, function(x) {
      as.matrix(x$concat.data)
    })
    ...
    if (is.null(misdata) & is.null(is.na.A) & is.null(ind.NA) &
      ...
      if (misdata.all) {
        ...
        ind.NA <- lapply(temp, function(x) {
          unique(x[, 1])
        })
        ind.NA.col <- lapply(temp, function(x) {
          unique(x[, 2])
        })
      } else {
        is.na.A <- NULL
    ...
    for (comp in 1:N)
    {
      ...
      for (ijk.keepA in 1:nrow(keepA[[comp]]))
      {
            ...
            if (misdata.all) # only one model, so misdata[1]=TRUE
              {
                ...
                if (length(ind.NA.col[[1]]) > 0) # should always be true
                  {
                    temp <- drop(variates.A[[1]][, comp]) %o% rep(
                      1,
                      length(ind.NA.col[[1]])
              ...
              mat.c[, comp] <- t(crossprod(
                variates.A[[1]][, comp],
                R[[1]]
              )) / drop(crossprod(variates.A[[1]][, comp]))
            }
          } else {
          mat.c <- NULL
        }
        ...
        if (N != 1 & comp != N) {
          defla.result <- defl.select(
            yy = mint.block.result$variates.A,
    ...
    if (one.model) {
      ...
      if (all.outputs) {
        ...
        AVE <- list(
          AVE_X = AVE_X, AVE_outer = AVE_outer,
          AVE_inner = AVE_inner
        )
    ...
    out <- list(
      A = A, indY = indY, ncomp = ncomp, mode = mode,
      keepA = keepA,
      variates = variates.A, loadings = loadings.A,
      variates.partial = if (is.null(tau)) {
        variates.partial.A
      },
      loadings.partial = if (is.null(tau)) {
        loadings.partial.A
      },
      loadings.star = loadings.Astar,
      names = list(
        sample = row.names(A[[1]]), colnames = lapply(A, colnames),
        blocks = names(A)
      ),
...
sparse.mint.block_iteration <- function(A, design, study = NULL, loadings.A,
  ...
  for (q in 1:J)
  {
    if (misdata[q]) {
      ...
      d.variates.A.norm <- rep(
        crossprod(loadings.temp),
  ...
  repeat {
    ...
    for (q in 1:J)
    {
      ...
      if (!is.null(penalty)) {
        loadings.A[[q]] <- sparsity(loadings.A[[q]],
          keepA = NULL,
          penalty = penalty[q]
        )
      } else {
    ...
    diff.value <- max(sapply(1:J, function(x) {
      crossprod(loadings.A[[x]] -
        loadings.A_old[[x]])
    }))
  ...
  result <- list(
    variates.A = variates.A, loadings.A = loadings.A, crit =
      crit[which(crit != 0)],
...
sparse.rgcca_iteration <- function(A, design, tau = "optimal", scheme = "horst",
  ...
  repeat {
    ...
    for (j in c(which.primal, which.dual))
    {
      ...
      if (!is.null(keepA) || !is.null(penalty)) {
        ...
        if (!is.null(keepA)) {
          loadings.A[[j]] <- sparsity(
            loadings.A = loadings.A[[j]],
  ...
  result <- list(
    variates.A = variates.A, loadings.A = loadings.A,
