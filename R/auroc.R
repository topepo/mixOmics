...
auroc.mixo_plsda <-
  function(object,
    ...
    res.predict <- predict.mixo_spls(object,
      newdata = newdata,
      dist = "max.dist", multilevel = multilevel
    )$predict
...
auroc.list <-
  function(
    ...
    for (idx in seq_len(length(object))) { # for each model
      ...
      res.predict <- predict.mixo_spls(obj,
        newdata = newdata,
        dist = "max.dist"
      )$predict
    ...
    if (is.null(title)) {
      title <- paste0("ROC Curve Using Comp(s): ", paste0(seq_len(roc.comp), collapse = ", "))
    }
...
auroc.mint.plsda <-
  function(object,
    ...
    # note here: the dist does not matter as we used the predicted scores only
    res.predict <- predict.mixo_spls(object,
      newdata = newdata, dist = "max.dist",
      multilevel = multilevel, study.test = study.test
    )$predict
...
auroc.mint.block.plsda <- function(
  ...
  res.predict <- predict.mixo_spls(object,
    newdata = newdata,
    study.test = study.test, dist = "max.dist", multilevel = multilevel
  )$predict
  block.temp <- names(res.predict[roc.block])
