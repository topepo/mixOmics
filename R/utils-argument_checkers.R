...
.check_ncomp <- function(ncomp, X, default = 2) {
  ...
  err_msg <- paste0(
    "'ncomp' must be a positive integer smaller than ",
...
.check_comp <- function(comp, ncomp, style = c("2d", "3d")) {
  ...
  err_msg <- paste0(
    "'comp' must be a positive integer of length ", len,
...
#' }
.check_character <- function(arg, len = 1, default = NULL) { # TODO use this throughout for title etc
  formals(stop)$call. <- FALSE
