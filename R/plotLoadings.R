...
plotLoadings.mixo_plsda <-
  function(object,
           ...
           ...) {
    # -- input checks
    check <- check.input.plotLoadings(
      object = object, block = block, subtitle = subtitle, size.name = size.name, size.legend = size.legend,
    ...
    if (is.null(contrib)) {
      if (plot) {
        plotLoadings.mixo_pls(
          object = object, block = block, comp = comp, ndisplay = ndisplay,
          size.name = size.name,
          name.var = name.var,
      ...
      for (i in 1:length(block))
      {
        ...
        if (is.null(legend.color)) {
          legend.color <- color.mixo(1:10)[1:nlevels(Y)]
        } # by default set to the colors in color.mixo (10 colors)
        ...
        if (plot) # condition if all we need is the contribution stats
          {
            ...
            if (legend) {
              ...
              legend(0.8, 1.1,
                col = legend.color[1:nlevels(Y)], legend = levels(Y), pch = 19,
...
plotLoadings.mint.pls <-
  function(object,
    ...
    if (any(study == "global")) {
      ...
      plotLoadings.mixo_pls(
        object = object, block = c("X", "Y"), comp = comp, ndisplay = ndisplay,
        size.name = size.name,
        name.var = name.var,
        name.var.complete = name.var.complete,
        title = title,
...
plotLoadings.mint.plsda <-
  function(object,
    ...
    if (any(study == "global")) {
      plotLoadings.mixo_plsda(
        object = object, contrib = contrib, method = method, block = "X", comp = comp, ndisplay = ndisplay,
        size.name = size.name,
      ...
      # -- input checks
      check <- check.input.plotLoadings(
        object = object, block = "X", size.name = size.name, size.legend = size.legend,
...
check.input.plotLoadings <- function(object,
  ...
  if (!inherits(object, "DA")) {
    object$indY <- length(object$names$blocks) + 1
  } # we don't want to remove anything in that case, and 3 is higher than the number of blocks which is 2
...
get.contrib.df <- function(Y,
  ...
  which.contrib <- data.frame(matrix(FALSE,
    ncol = nlevels(Y) + 2, nrow = length(colnames.X),
