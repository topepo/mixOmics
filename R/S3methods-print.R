...
print.pca <- function(x, ...) {
  ...
  cat(
    "  Proportion of", var.type, "explained variance for the first", ind.show,
...
print.sgcca <- function(x, ...) {
  ...
  list.select <- list()
  for (k in 1:length(x$X)) {
    list.select[[k]] <- apply(x$loadings[[k]], 2, function(x) {
      sum(x != 0)
    })
...
print.tune.rcc <- function(x, ...) {
  cat("\nCall:\n", deparse(x$call, width.cutoff = 500), "\n\n")
  cat(
    "  lambda1 = ", x$opt.lambda1, ", see object$opt.lambda1\n", " lambda2 = ", x$opt.lambda2, ",  see object$opt.lambda2\n",
