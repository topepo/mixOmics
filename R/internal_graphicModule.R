  ...
  function(df,
    ...
    if (style == "ggplot2") {
      ...
      if (!is.null(background)) {
        for (i in 1:length(background))
        {
          if (!is.null(background[[i]])) {
            background[[i]] <- data.frame(
              id = i, col = names(background)[i],
              background[[i]]
            )
          }
        }
      ...
      p <- p + scale_color_manual(
        values = unique(col.per.group)[match(
    ...
    if (style == "lattice") {
      ...
      p <- xyplot(y ~ x | Block,
        data = df, xlab = list(
          label = X.label,
          cex = size.xlabel
        ), ylab = list(label = Y.label, cex = size.ylabel),
        main = list(label = title, cex = size.title), as.table = TRUE,
        # as.table plot in order
        groups = if (display.names) {
          names
        } else {
          group
        },
        scales = list(
          x = list(relation = "free", limits = xlim, cex = size.axis),
        ...
        key = if (legend == TRUE) {
          if (!any(class.object %in% object.mint)) {
            if (group.pch == "same") {
              list(
                space = legend.position, title = legend.title,
                ...
                pch = if (display.names | any(class.object %in% object.mint)) {
                  ...
                  unique(df$pch.legend)
                }, text =
                  list(levels(df$group))
              )
            } else {
              list(
                space = legend.position, cex.title = size.legend.title,
                point = list(
                  col = c(NA, col.per.group, NA, NA, rep(
                    "black",
                    nlevels(df$pch.levels)
                  ))
                ),
                cex = c(
                  size.legend.title, rep(
                    size.legend,
                    length(col.per.group)
                  ), size.legend, size.legend.title,
                  rep(size.legend, nlevels(factor(df$pch)))
                ),
                pch = c(
                  NA, rep(16, length(col.per.group)), NA, NA,
                  values.pch
                ),
                text = list(outcome = c(
                  legend.title, levels(df$group),
        ...
        panel = function(x, y, subscripts, groups, display = display.names, ...) {
          ...
          if (!is.null(background)) # only first block: plsda and splsda
            {
              for (i in 1:length(background)) {
                panel.polygon(background[[i]],
                  col = names(background)[i],
                  border = NA
                )
              }
            }
          ...
          for (i in 1:nlevels(df$group))
          {
            if (display) {
              ltext(
                x = df$x[df$group == levels(df$group)[i]],
          ...
          for (i in unique(df$col))
          {
            if (display) {
              ltext(
                x = df$x[subscripts][df$col[subscripts] == i],
      ...
      if (centroid) {
        ...
        for (k in 1:nlevels(df$Block))
        {
          ...
          for (i in 1:nlevels(df$group))
          {
            ...
            panel.points(
              x = x0, y = y0, col = unique(col.per.group)[i],
      ...
      if (ellipse) {
        ...
        for (k in 1:nlevels(df$Block))
        {
          ...
          {
            panel.lines(
              x = df.ellipse[other.ellipse, paste0(
                "Col",
                2 * (i - 1) + 1
              )],
      ...
      for (k in 1:nlevels(df$Block))
      {
        if (legend & group.pch == "same") {
          ...
          par(mai = c(
            1.360000, 1.093333, 1.093333,
            (max(strwidth(
              c(levels(df$group), legend.title, legend.title.pch),
              "inches"
            ))) + 1
          ), xpd = TRUE)
        }
        ...
        plot(df[other, "x"], df[other, "y"],
          ...
          xlim = c(xlim[[k]][1], xlim[[k]][2]), ylim = c(
            ylim[[k]][1],
            ylim[[k]][2]
          ),
        ...
        #-- initialise plot
        if (nlevels(df$Block) == 1 & !any(class.object %in% c(
          object.mint,
          "sgcca", "rgcca"
        )))
        ...
        for (i in 1:nlevels(df$group))
        {
          if (display.names) {
            text(
              x = df[df$group == levels(df$group)[i] & other, "x"],
              y = df[df$group == levels(df$group)[i] & other, "y"],
              labels = df[
                df$group == levels(df$group)[i] & other,
                "names"
              ],
        ...
        if (legend & group.pch == "same") {
          ...
          col = col.per.group,
          legend = levels(df$group), pch = if (display.names) {
            16
          } else {
            unique(df$pch.legend)
          }, title = legend.title, cex = size.legend,
          lty = 0, lwd = point.lwd
          )
        } else if (legend & group.pch == "different") {
          legend(par()$usr[2] + 0.1, par()$usr[4] -
        ...
        if (legend) {
          par(xpd = FALSE)
        } # so the abline does not go outside the plot
          ...
          for (i in 1:nlevels(df$group))
          {
              ...
              other, "x"]))
            {
              ...
              cex = df$df[
                df$group == levels(df$group)[i] & other,
                "cex"
              ], col = df[df$group == levels(df$group)[i] &
    ...
    if (style == "3d") {
      ...
      for (k in 1:nlevels(df$Block))
      {
        ...
        if (any(class.object %in% c(
          "ipca", "sipca", "pca", "spca", "prcomp",
          ...
          if (display.names) {
            ...
            for (pch_i in unique(df[df$col == i, ]$pch))
            {
              ...
              if (pch_i == "sphere") {
                ...
                rgl::shapelist3d(rgl::tetrahedron3d(),
                  x = df[df$col == i &
                    other, "x"][ind],
                  y = df[df$col == i & other, "y"][ind],
                  z = df[df$col == i & other, "z"][ind],
                ...
                rgl::shapelist3d(rgl::octahedron3d(),
                  x = df[df$col == i &
                    other, "x"][ind],
                  y = df[df$col == i & other, "y"][ind],
                  z = df[df$col == i & other, "z"][ind],
                ...
                rgl::shapelist3d(rgl::icosahedron3d(),
                  x = df[df$col == i &
                    other, "x"][ind],
                  y = df[df$col == i & other, "y"][ind],
                  z = df[df$col == i & other, "z"][ind],
                ...
                rgl::shapelist3d(rgl::dodecahedron3d(),
                  x = df[df$col == i &
                    other, "x"][ind],
                  y = df[df$col == i & other, "y"][ind],
                  z = df[df$col == i & other, "z"][ind],
        ...
        if (centroid == TRUE) {
          for (i in 1:nlevels(df$group))
          {
            ...
            rgl::points3d(
              x = x0, y = y0, z = z0, cex = df$df[df$group ==
        ...
        if (star == TRUE) {
          for (i in 1:nlevels(df$group))
          {
            ...
            {
              rgl::segments3d(
                x = c(x0, df[df$group == levels(df$group)[i] &
