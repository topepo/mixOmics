...
#'   X = data, Y = type.id, ncomp = 2, near.zero.var = FALSE,
...
#' out <- tune.mint.splsda(
#'   X = data, Y = type.id, ncomp = 2, study = exp,
...
tune.mint.splsda <-
  function(X,
    ...
    check <- Check.entry.pls(X,
      Y = Y.mat, ncomp = ncomp, mode = "regression", scale = scale,
    ...
    mat.sd.error <- matrix(0,
      nrow = length(test.keepX), ncol = ncomp - length(already.tested.X),
      dimnames = list(c(test.keepX), c(paste0("comp", comp.real)))
    )
    mat.mean.error <- matrix(
      nrow = length(test.keepX), ncol = ncomp - length(already.tested.X),
      dimnames = list(c(test.keepX), c(paste0("comp", comp.real)))
    )

    error.per.class.mean <- matrix(
      nrow = nlevels(Y), ncol = ncomp - length(already.tested.X),
      dimnames = list(c(levels(Y)), c(paste0("comp", comp.real)))
    )
    error.per.class.sd <- matrix(0,
      nrow = nlevels(Y), ncol = ncomp - length(already.tested.X),
      dimnames = list(c(levels(Y)), c(paste0("comp", comp.real)))
    )


    error.per.study.keepX.opt <- matrix(
      nrow = nlevels(study), ncol = ncomp - length(already.tested.X),
    ...
    for (comp in 1:length(comp.real))
    {
      ...
      result <- LOGOCV(X, Y,
        ncomp = 1 + length(already.tested.X), study = study,
