...
#' tune <- tune.splsda(X, Y,
#'   ncomp = 1, nrepeat = 10, logratio = "none",
...
#'
#' tune <- tune.splsda(X,
#'   Y = Y, ncomp = 3, nrepeat = 10, logratio = "none",
...
tune.splsda <-
  function(X, Y,
           ...
           cpus = 1) { #-- checking general input parameters --------------------------------------#
    #---------------------------------------------------------------------------#
    ...
    if (is.null(multilevel) | (!is.null(multilevel) && ncol(multilevel) == 2)) {
      ...
      if (all(measure != "AUC")) {
        error.per.class.keepX.opt.mean <- matrix(0,
          nrow = nlevels(Y), ncol = length(comp.real),
      ...
      for (comp in 1:length(comp.real))
      {
        ...
        result <- MCVfold.spls(X, Y,
          multilevel = multilevel, validation = validation, folds = folds, nrepeat = nrepeat, ncomp = 1 + length(already.tested.X),
