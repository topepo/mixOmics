...
perf.mint.plsda <- function(object,
  ...
  keepX <- apply(object$loadings$X, 2, function(x) {
    sum(x != 0)
  })
  ...
  for (ijk in dist)
  {
    class.all[[ijk]] <- matrix(
      nrow = nrow(X), ncol = ncomp,
  ...
  for (study_i in 1:nlevels(study)) # LOO on the study factor
  {
    ...
    study.specific[[study_i]]$BER <- global$BER <- matrix(0,
      nrow = ncomp, ncol = length(dist),
      dimnames = list(c(paste0("comp", 1:ncomp)), dist)
    )

    study.specific[[study_i]]$overall <- global$overall <- matrix(0,
      nrow = ncomp, ncol = length(dist),
      dimnames = list(c(paste0("comp", 1:ncomp)), dist)
    )
    ...
    for (ijk in dist) {
      study.specific[[study_i]]$error.rate.class[[ijk]] <- global$error.rate.class[[ijk]] <- matrix(0,
  ...
  for (comp in 1:ncomp)
  {
    ...
    class.comp <- list()
    for (ijk in dist) {
      class.comp[[ijk]] <- matrix(0, nrow = nrow(X), ncol = 1)
    ...
    for (study_i in 1:M) # LOO on the study factor
    {
      ...
      object.res <- mint.splsda(X.train, Y.train,
        study = study.learn.CV, ncomp = comp,
      ...
      for (ijk in dist) {
        class.comp[[ijk]][omit, 1] <- test.predict.sw$class[[ijk]][, comp]
  ...
  result <- list(
    study.specific.error = study.specific,
    global.error = global,
  ...
  if (nlevels(study) > 2 & ncomp > 1) {
    ...
    for (measure_i in measure)
    {
      for (ijk in dist)
      {
        mat.error.rate <- sapply(study.specific, function(x) {
          x[[measure_i]][, ijk]
        })
