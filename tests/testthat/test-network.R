...
test_that("network works for rcc", {
  ...
  network.rcc.res <- network(nutri.res,
    comp = 1:3, cutoff = 0.6, save = "jpeg",
    name.save = tmp.file
  )
...
test_that("network works for spls", {
  ...
  ## create a tmp file
  tmp.file <- tempfile("network", fileext = ".jpeg")
  network.spls.res <- network(toxicity.spls,
    comp = 1:3, cutoff = 0.8,
...
test_that("network works when `shape.node` == 'none'", {
  data(breast.TCGA)
  X <- list(
    mRNA = breast.TCGA$data.train$mrna[, 1:100],
