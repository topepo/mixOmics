...
.get.group <- function(group, object = NULL, n_ind = 0) {
  ...
  if (is.null(out)) {
    if (is(object, "DA")) {
      ...
      if (n_ind != length(out)) {
        stop(
          "Unexpected error. Please submit an issue with reproducible example to\n",
...
.get.cols.and.group <- function(
  ...
  if (!all(names(col.per.group) %in% levels(group))) {
    stop(
      "'col.per.group' should be a named vector whose names match group (",
      levels(group), ")"
    )
  }
  ...
  if (is.null(group)) {
    ...
    col.per.group <-
      .change_if_null(
        col.per.group,
        color.mixo(
          seq_len(1 + nlevels(group))[-3] ## bc 3rd one is grey
        )
      )
    names(col.per.group) <- levels(group)
