...
cim <-
  function(mat = NULL,
    ...
    clust.method <- choices[c(
      pmatch(clust.method[1], choices),
    ...
    dist.method <- choices[c(
      pmatch(dist.method[1], choices),
    ...
    if (any(class.object %in% object.all)) {
      ...
      if (any(class.object %in% object.block.pls)) {
        ...
        if (length(unique(sapply(1:length(mat$X), FUN = function(x) {
          nrow(mat$X[[x]])
        }))) != 1) {
      ...
      if (any(class.object %in% object.single.omics)) {
        ...
        if (any(class.object %in% c(
          ...
          if (any(class.object %in% c("mixo_splsda", "mixo_mlsplsda"))) {
            keep.X <- apply(
              abs(mat$loadings$X[, comp, drop = FALSE]), 1,
              sum
            ) > 0
          } else {
        ...
        if (any(class.object %in% object.block.pls)) {
          ...
          if (mapping == "multiblock") {
            if (mat$mode == "canonical") {
              ...
              for (b in 2:length(rel.blocks.variates)) {
                bisect <- bisect + rel.blocks.variates[[b]][, comp]
              }
              ...
              cord.X <- cor(do.call("cbind", rel.blocks.X)[, keep.X, drop = FALSE],
                do.call("cbind", rel.blocks.variates)[, comp],
                use = "pairwise"
              )
            }
          } else {
            if (mat$mode == "canonical") {
              ...
              cord.X <- cor(mat$X[[X.block.name]][, keep.X, drop = FALSE],
                mat$variates[[X.block.name]][, comp],
                use = "pairwise"
              )
              cord.Y <- cor(mat$X[[Y.block.name]][, keep.Y, drop = FALSE],
                mat$variates[[Y.block.name]][, comp],
                use = "pairwise"
              )
            }
          }
          ...
          if (any(class.object %in% c("mixo_spls", "mixo_mlspls"))) {
            ...
            keep.Y <- apply(
              abs(mat$loadings$Y[, comp, drop = FALSE]), 1,
              sum
            ) > 0
          } else {
            keep.X <- apply(abs(mat$loadings$X), 1, sum) > 0
        ...
        if (mapping == "multiblock") {
          ...
          if (!is.null(col.sideColors)) {
            ...
            idx <- unlist(c(sapply(1:length(rel.blocks.X), function(x) {
              rep(x, num.keep.X[[x]])
            })))
