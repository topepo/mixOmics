...
spca <-
  function(X,
    ...
    if (isFALSE(center) && any(colMeans(X, na.rm = TRUE) != 0)) {
      warning(
        "data contain missing values which will be ",
    ...
    for (h in 1:ncomp) {
      ...
      repeat{
        ...
        if (crossprod(u - u.old) < tol) {
          break
        }
        if (crossprod(loadings - loadings.old) < tol) {
          break
        }
...
.eval_non.orthogonality <- function(variates,
  ...
  if (non_orthog >= code_red_threshold) {
    warning(
      "The sPCA algorithm failed to extract orthogonal components. ",
      recom_scale_msg
    )
  } else if (non_orthog >= warn_threshold & non_orthog < code_red_threshold) {
    warning(
      "The extracted components are not completely orthogonal. ",
