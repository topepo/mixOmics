...
rcc <-
  function(X,
    ...
    if (method == "ridge") {
      ...
      Cxy <- h1 * crossprod(
        sweep(sweep(xs, 1, sqrt((1 - lambda.x) * w), "*"), 2, sc.x, "*"),
    ...
    result <- list(
      ...
      names = list(
        sample = ind.names, colnames = list(X = colnames(X), Y = colnames(Y)), blocks = c("X", "Y"), # list(X = X.names, Y = Y.names, indiv = ind.names,
        data = data.names
      ),
      lambda = lambda
    )
