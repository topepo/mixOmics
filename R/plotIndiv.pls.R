...
plotIndiv.mixo_pls <-
  function(object,
    ...
    check <- check.input.plotIndiv(
      object = object, comp = comp, blocks = blocks, ind.names = ind.names,
    ...
    #-- get the variates
    variate <- internal_getVariatesAndLabels(object, comp,
      blocks = blocks, rep.space = rep.space, style = style, X.label = X.label,
    ...
    out <- shape.input.plotIndiv(
      object = object, n = n, blocks = blocks, x = x, y = y, z = z, ind.names = ind.names, group = group,
    ...
    res <- internal_graphicModule(
      df = df, centroid = centroid, col.per.group = col.per.group, title = title,
