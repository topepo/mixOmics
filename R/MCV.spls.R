...
MCVfold.spls <- function(
    ...
    # ind.NA,
    # ind.NA.col,
    parallel) { #-- checking general input parameters --------------------------------------#
  #---------------------------------------------------------------------------#
  ...
  if (ncomp > 1 & any(class.object == "DA")) {
  ...
  if (any(class.object == "DA")) {
    for (ijk in dist) {
      class.comp[[ijk]] <- array(0, c(nrow(X), nrepeat, length(test.keepX)))
    ...
    fonction.j.folds <- function(j) # for (j in 1:M)
    {
      ...
      if (any(misdata)) {
        if (any(class.object == "DA")) {
          ...
          temp <- lapply(is.na.A.train, function(x) {
            which(x, arr.ind = TRUE)
          })

          ind.NA.train <- lapply(temp, function(x) {
            unique(x[, 1])
          })
          ind.NA.col.train <- lapply(temp, function(x) {
            unique(x[, 2])
          })
        }
      ...
      if (any(class.object == "DA")) {
        ...
        for (ijk in dist) {
          class.comp.j[[ijk]] <- matrix(0, nrow = length(omit), ncol = length(test.keepX))
        } # prediction of all samples for each test.keepX and  nrep at comp fixed
      } else {
      ...
      result <- suppressWarnings(internal_wrapper.mint(
        X = X.train, Y = Y.train.mat, study = factor(rep(1, nrow(X.train))), ncomp = ncomp,
      ...
      if (scale) {
        sigma.Y <- matrix(attr(result$A[[2]], "scaled:scale"), nrow = nrow(X.test), ncol = q, byrow = TRUE)
      } else {
        sigma.Y <- matrix(1, nrow = nrow(X.test), ncol = q)
      }
      ...
      for (i in seq_len(nrow(test.keepA)))
      {
        ...
        result$variates <- lapply(result.temp$variates, function(x) {
          if (ncol(x) != ncomp) {
            x[, colnames(x) %in% names.to.pick, drop = FALSE]
          } else {
            x
          }
        })
        ...
        if (any(class.object == "DA")) {
          for (ijk in dist) {
            class.comp.j[[ijk]][, i] <- test.predict.sw$class[[ijk]][, ncomp]
  ...
  if (any(measure == "MSE")) {
    ...
    varY <- apply(Y, 2, var)
    error <- lapply(prediction.keepX, function(z) {
      apply(z, c(3), function(x) {
        temp <- (Y - x)^2
        temp2 <- t(t(temp) / varY)
    ...
    mat.error.final[[ijk]] <- lapply(error, function(x) {
      x / nrow(Y)
    }) # percentage of misclassification error for each test.keepX (rows) and each nrepeat (columns)

    # we want to average the error per keepX over nrepeat and choose the minimum error
    error.mean[[ijk]] <- sapply(error, mean) / nrow(Y)
    if (!nrepeat == 1) {
      error.sd[[ijk]] <- sapply(lapply(error, function(x) {
        apply(matrix(x, nrow = ncol(Y)), 1, sd) / nrow(Y)
      }), mean)
    }
  ...
  if (any(measure == "R2")) # R2 (square of the correlation of the truth and the predicted values, averaged over the columns of Y)
    {
      ...
      if (!nrepeat == 1) {
        error.sd[[ijk]] <- sapply(lapply(error, function(x) {
          apply(matrix(x, nrow = ncol(Y)), 1, sd)
        }), mean)
      }
