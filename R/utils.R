...
.plotLoadings_barplot <- function(height, col, names.arg, cex.name, border, xlim) {
  tryCatch(
    {
      barplot(height,
        horiz = TRUE, las = 1, col = col, axisnames = TRUE, names.arg = names.arg, # names.arg = row.names(df),
...
.create_design <- function(X, design = "full", indY = NULL) {
  ...
  if (is.character(design)) {
    ...
    stop(
      "'design' must be a matrix, or one of c('full', 'null'), or a numeric ",
