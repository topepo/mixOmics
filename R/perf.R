...
.perf.mixo_pls_cv <- function(object,
  ...
  if (mode == "canonical") {
    ...
    # RSS.indiv[[1]] = X
    press.mat <- lapply(1:ncomp, function(x) {
  ...
  for (i in 1:M)
  {
    ...
    for (h in 1:ncomp)
    {
      ...
      spls.res <- spls(X.train[, nzv.X, drop = FALSE], Y.train[, nzv.Y, drop = FALSE],
        ncomp = 1, mode = mode, max.iter = max.iter, tol = tol,
  ...
  for (h in 1:ncomp) {
    ...
    PRESS.inside[h, ] <- apply(press.mat[[h]], 2, function(x) {
      norm(x, type = "2")^2
    })
...
perf.mixo_plsda <- function(object,
  ...
  {
    mat.sd.error[[measure_i]] <- matrix(0,
      nrow = ncomp, ncol = length(dist),
      dimnames = list(c(paste0("comp", 1:ncomp)), dist)
    )
    mat.mean.error[[measure_i]] <- matrix(0,
      nrow = ncomp, ncol = length(dist),
      dimnames = list(c(paste0("comp", 1:ncomp)), dist)
    )
    ...
    for (ijk in dist)
    {
      mat.error.rate[[measure_i]][[ijk]] <- matrix(0,
        nrow = ncomp, ncol = nrepeat,
      ...
      error.per.class.keepX.opt.mean[[measure_i]][[ijk]] <- matrix(
        nrow = nlevels(Y), ncol = ncomp,
  ...
  for (comp in 1:ncomp)
  {
    ...
    result <- MCVfold.spls(X, Y,
      multilevel = multilevel, validation = validation, folds = folds, nrepeat = nrepeat, ncomp = comp,
    ...
    prediction.all[[comp]] <- array(unlist(result$prediction.comp), c(nrow(result$prediction.comp[[1]]), ncol(result$prediction.comp[[1]]), nrepeat),
      dimnames = c(dimnames(result$prediction.comp[[1]])[1:2], list(paste0("nrep", 1:nrepeat)))
    ) # [[1]][, , 1] #take only one component [[1]] and one of test.keepX [,,1]
  ...
  ncomp_opt <- matrix(NA,
    nrow = length(measure), ncol = length(dist),
