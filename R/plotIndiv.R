...
internal_getVariatesAndLabels <-
  function(object,
    ...
    if (any(class.object %in% c(object.pls, object.blocks, object.mint))) {
      x <- lapply(object$variates, function(x) {
        x[, comp[1], drop = FALSE]
      })
      y <- lapply(object$variates, function(x) {
        x[, comp[2], drop = FALSE]
      })
      if (style == "3d") {
        z <- lapply(object$variates, function(x) {
          x[, comp[3], drop = FALSE]
        })
      } else {
        z <- NULL
...
shape.input.plotIndiv <-
  function(object,
    ...
    if (ellipse) {
      ...
      variance <- lapply(1:length(x), function(z) {
        lapply(matrice[[z]], var)
      })
      ...
      max.ellipse <- lapply(1:length(x), function(z1) {
        sapply(coord.ellipse[[z1]], function(z2) {
          apply(z2, 2, max)
        })
      })
      ...
      if (is.null(xlim)) {
        xlim <- lapply(1:length(x), function(z) {
          c(min(x[[z]], min.ellipse[[z]][1, ]), max(x[[z]], max.ellipse[[z]][1, ]))
        })
      }
      if (is.null(ylim)) {
        ylim <- lapply(1:length(x), function(z) {
          c(min(y[[z]], min.ellipse[[z]][2, ]), max(y[[z]], max.ellipse[[z]][2, ]))
        })
      }
    ...
    if (missing(pch) & !any(class.object %in% object.mint)) {
      if (style == "3d") {
        pch <- unlist(lapply(1:length(length(levels(group))), function(x) {
          rep(c("sphere", "tetra", "cube", "octa", "icosa", "dodeca")[x], length(group == x))
        }))
      } else {
    ...
    if (any(study == "global")) # | length(study) == 1)
      {
        ...
        if (length(blocks) == 1 & !any(class(object) %in% c(object.mint, "sgcca", "rgcca"))) # add blocks == 1 to allow "multi" with plsda
          ...
          df <- data.frame(do.call(rbind, df), "Block" = paste0("Block: ", unlist(lapply(1:length(df), function(z) {
            rep(blocks[z], nrow(df[[z]]))
          }))))
        ...
        if (ellipse == TRUE) {
          df.ellipse <- data.frame(do.call("rbind", lapply(1:length(x), function(k) {
            do.call("cbind", coord.ellipse[[k]])
          })), "Block" = paste0("Block: ", rep(blocks, each = 100)))
          if (length(ind.unique) > 0) {
      ...
      df <- data.frame(do.call(rbind, df), "Block" = paste0("Study: ", unlist(lapply(1:length(df), function(z) {
        rep(blocks[z], nrow(df[[z]]))
      }))))
      df$Block <- factor(df$Block, levels = unique(df$Block))
    ...
    plot_parameters <- list(
      size.title = size.title, size.subtitle = size.subtitle, size.xlabel = size.xlabel, size.ylabel = size.ylabel,
