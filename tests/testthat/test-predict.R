...
test_that("predict.block.splsda", code = {
  ...
  # example with block.splsda=diablo=sgccda and a missing block
  res.train <- block.splsda(
    X = list(gene = gene.train, lipid = lipid.train), Y = Y.train,
...
test_that("predict.mint.splsda works", code = {
  ...
  res <- mint.splsda(
    X = gene.train, Y = Y.train, ncomp = 3, keepX = c(10, 5, 15),
    study = study.train
  )
...
test_that("(predict:error): catches when colnames(newdata) differs from colnames(X)", {
  ...
  # set design matrix
  design <- matrix(0.1,
    ncol = length(X.train), nrow = length(X.train),
  ...
  final.diablo.model <- block.splsda(
    X = X.train, Y = Y.train, ncomp = optimal.ncomp, # set the optimised DIABLO model
