...
Check.entry.pls <- function(X, Y, ncomp, keepX, keepY, test.keepX, test.keepY,
  ...
  if (mode == "canonical" & ncomp > ncol(Y)) {
    stop(
      "For `canonical mode', 'ncomp' needs to be lower than ncol(Y)= ",
      ncol(Y)
    )
  }
  ...
  if (ncomp > P) {
    warning(
      "Reset maximum number of variates 'ncomp' to ncol(X) = ", P,
      "."
    )
    ncomp <- P
  }
  ...
  return(list(
    X = X, Y = Y, ncomp = ncomp, X.names = X.names, Y.names = Y.names,
...
Check.entry.wrapper.mint.block <- function(X,
  ...
  if (!missing(Y)) # Y is not missing, so we don't care about indY
    {
      ...
      if (missing(design)) {
        ...
        stop(paste0(
          "'design' must be a square matrix with ", length(X),
          "columns."
        ))
    ...
    if (missing(design)) {
      ...
      {
        stop(paste0(
          "'design' must be a square matrix with ", length(A),
          "columns."
        ))
      }
    ...
    if (!is.numeric(indY) | indY > length(A) | length(indY) > 1) {
      stop(paste0(
        "'indY' must be a numeric value lower or equal to ",
  ...
  check <- sapply(1:length(A), function(j) {
    identical(
      ind.names[[1]],
      ind.names[[j]]
    )
  ...
  return(list(
    A = A, ncomp = ncomp, study = study, keepA = keepA,
...
Check.entry.sgcca <- function(X,
  ...
  for (q in 1:length(X))
  {
    ...
    if (ncomp[q] > ncol(X[[q]])) {
      warning(paste0(
        "Reset maximum number of variates 'ncomp[", q,
        "]' to ncol(X[[", q, "]])= ", ncol(X[[q]]), "."
      ))
      ncomp[q] <- ncol(X[[q]])
    }
  ...
  return(list(
    A = A, ncomp = ncomp, design = design, init = init, scheme = scheme,
