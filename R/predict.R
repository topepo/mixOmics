  ...
  function(object,
    ...
    if (!inherits(object, block.object)) # not a block (pls/spls/plsda/splsda/mint...)
      {
        ...
        } # this should never happen/intern check
        ...
        if (length(dim(newdata)) == 2) {
          if (ncol(newdata) != p) {
            stop(
              "'newdata' must be a numeric matrix with ncol = ", p,
      ...
      if (!is.null(object$nzv)) {
        ...
        for (x in 1:length(newdata)) {
          if (nrow(object$nzv[[x]]$Metrics) == 0) {
            next
          }
          if (all(!(rownames(object$nzv[[x]]$Metrics) %in% colnames(newdata[[x]])))) {
            next
          }
        ...
        if (any(X.names != newdata.names)) { # if there is any difference between colnames
          if (length(setdiff(X.names, newdata.names)) > 0) { # if there is presence of novel feature/absence of existing feature
            stop(paste("The set of features in 'object$X$",
              ...
              names(newdata)[i], "'. Please ensure they have the same features.",
              sep = ""
            ))
            ...
            stop(paste("The order of features in 'object$X$",
              names(X)[i], "' is different to 'newdata$",
              names(newdata)[i], "'. Please ensure that you adjust these to the same order",
              sep = ""
            ))
          }
        }
      ...
      if (all.equal(lapply(newdata, colnames), lapply(X, colnames)) != TRUE) {
        newdata <- setNames(lapply(seq_along(X), function(i) {
          newdata[[i]][, colnames(X[[i]])]
        }), nm = names(X))
      }
      ...
      if (!inherits(object, mint.object)) #| nlevels(factor(object$study))<=1) #not a mint object or just one level in the study
        { # not a mint (pls/spls/plsda/splsda/block...)
          ...
          if (!is.null(attr(X[[1]], "scaled:center"))) {
            newdata[which(!is.na(ind.match))] <- lapply(which(!is.na(ind.match)), function(x) {
              sweep(newdata[[x]], 2, STATS = attr(X[[x]], "scaled:center"))
            })
          }
          if (scale) {
            newdata[which(!is.na(ind.match))] <- lapply(which(!is.na(ind.match)), function(x) {
              sweep(newdata[[x]], 2, FUN = "/", STATS = attr(X[[x]], "scaled:scale"))
            })
          }
          if (any(unlist(lapply(newdata[which(!is.na(ind.match))], function(x) {
            any(is.infinite(x))
          })))) {
            newdata[which(!is.na(ind.match))] <- lapply(which(!is.na(ind.match)), function(x) {
              df <- newdata[[x]]
              df[which(is.infinite(df))] <- NaN
              return(df)
            })
          }
          ...
          if (scale) {
            sigma.Y <- matrix(attr(Y, "scaled:scale"), nrow = nrow(newdata[[1]]), ncol = q, byrow = TRUE)
          } else {
    ...
    if ((hasArg(misdata.all) & hasArg(is.na.X)) && any(list(...)$misdata.all)) # ind.na.X: all blocks except Y
      { # if misdata.all and ind.na.X are provided, we don't calculate the is.na(X) as it takes time. Used in tune functions.
        for (j in c(1:J)[list(...)$misdata.all]) {
          X[[j]][list(...)$is.na.X[[j]]] <- 0
        } # faster than using replace
      } else {
      # replace NA by 0
    ...
    if ((hasArg(misdata.all) & hasArg(is.na.newdata)) && any(list(...)$misdata.all)) # ind.na.newdata: all blocks of newdata
      {
        ...
        concat.newdata <- lapply(1:J, function(q) {
          replace(concat.newdata[[q]], list(...)$is.na.newdata[[q]], 0)
        })
      }
    ...
    for (i in 1:J)
    {
      ...
      Ypred <- lapply(1:ncomp[i], function(x) {
        concat.newdata[[i]] %*% Wmat[, 1:x] %*% solve(t(Pmat[, 1:x]) %*% Wmat[, 1:x]) %*% t(Cmat)[1:x, ]
      })
      ...
      B.hat[[i]] <- sapply(1:ncomp[i], function(x) {
        Wmat[, 1:x] %*% solve(t(Pmat[, 1:x]) %*% Wmat[, 1:x]) %*% t(Cmat)[1:x, ]
      }, simplify = "array")
    ...
    if (is(object, "DA")) # a DA analysis (mint).(block).(s)plsda
      {
        if (inherits(object, block.object) & length(object$X) > 1) {
          if (!hasArg(noAveragePredict)) {
            ...
            out$AveragedPredict.class$max.dist <- matrix(sapply(
              1:ncomp[1], ### List level
              function(y) {
                apply(
                  out$AveragedPredict[, , y, drop = FALSE], 1, ### component level
                  function(z) {
            ...
            out$WeightedPredict.class$max.dist <- matrix(sapply(
              1:ncomp[1], ### List level
              function(y) {
                apply(
                  out$WeightedPredict[, , y, drop = FALSE], 1, ### component level
                  function(z) {
                    paste(levels(Y.factor)[which(z == max(z))], collapse = "/")
                  }
                ) ### matrix level
