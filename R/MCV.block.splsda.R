...
MCVfold.block.splsda <-
  function(X,
    ...
    if (validation == "Mfold") {
      ...
      M <- n
      if (nrepeat != 1) {
        stop("nrepeat should be set to 1 with validation='loo'\n")
      }
    }
    ...
    repeat_cv <- function(nrep, all_folds = all_folds, M = M) {
      ...
      fonction.j.folds <- function(j) # for(j in 1:M)
      {
        ...
        var.train <- lapply(X.train, function(x) {
        ...
        if (!is.null(attr(result$A[[1]], "scaled:center"))) {
          X.test[which(!is.na(ind.match))] <- lapply(
            which(!is.na(ind.match)),
            function(x) {
              sweep(X.test[[x]], 2,
                STATS =
        ...
        for (i in 1:nrow(test.keepA))
        {
          ...
          names.to.pick.ncomp <- paste(paste0("comp", ncomp),
            paste(as.numeric(keepA[[ncomp]][i, ]), collapse = "_"),
            sep = ":"
          )
