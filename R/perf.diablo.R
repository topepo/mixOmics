  ...
  function(object,
    ...
    repeat_cv_perf.diablo <- function(nrep) {
      ...
      X.training <- lapply(folds, function(x) {
        ...
        names(out) <- names(X)
        out
      }) # need to name the block for prediction
      ...
      block.splsda.args <- function(ind) {
        return(list(
          X = X.training[[ind]], Y = Y.training[[ind]], ncomp = ncomp,
          keepX = keepX,
      ...
      features <- lapply(1:J, function(x) {
        lapply(
          1:object$ncomp[x], ### List level / ncomp level
          function(y) {
            unlist(lapply(
              1:M, ### Validation level
              function(z) {
      ...
      for (prediction_framework in names(ncomp_opt))
      {
        if (prediction_framework %in% c("AveragedPredict", "WeightedPredict")) {
          ncomp_opt[[prediction_framework]] <- matrix(NA,
            nrow = length(measure), ncol = 1,
            dimnames = list(measure)
          )

          for (measure_i in measure)
          {
            mat.error.rate <- sapply(get(paste0(prediction_framework, ".error.rate.all")), function(x) {
              x[measure_i, ]
            })
