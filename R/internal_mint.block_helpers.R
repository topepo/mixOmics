...
# problem: divide by n instead of n-#NA
{
  ...
  out <- list(
    data_scale = data.list.study.scale_i, meanX = meanX,
    sqrt.sdX = sqrt.sdX
  )
  return(out)
}
...
mean_centering_per_study <- function(data, study, scale) {
  ...
  meanX <- lapply(res, function(x) {
    x[[2]]
  })
  sqrt.sdX <- lapply(res, function(x) {
    x[[3]]
  })
  rownames.study <- lapply(res, function(x) {
    rownames(x[[1]])
  })
  ...
  if (M > 1) # otherwise already same order
    {
      concat.data <- do.call("rbind", lapply(res, function(x) {
        x[[1]]
      }))
...
initialisation_by_svd <- function(A, indY = NULL, misdata, is.na.A = NULL,
  ...
  if (init == "svd") {
    ...
    M <- lapply(c(seq_len(J))[-indY], function(x) {
      crossprod(A[[x]], A[[indY]])
    })
    # ssvd faster with svds, only if more than 3 column, otherwise break down
    svd.M <- lapply(M, function(x) {
      if (ncol(x) > 3) {
        svds(x, k = 1, nu = 1, nv = 1)
      } else {
        svd(x, nu = 1, nv = 1)
      }
    })
    ...
    alpha <- lapply(seq_len(J), function(y) {
      initsvd(A[[y]])
    })
