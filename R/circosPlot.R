...
.circosPlot <-
  function(object,
    ...
    if (!is.null(blocks)) { ## allow selection of blocks to show
    ...
    if (missing(color.cor)) {
      color.cor <- c(
        colors()[134], # blue, negative correlation
        colors()[128]
      ) # pale red, positive correlation
    } else {
    ...
    if (any(comp > min(object$ncomp))) {
      warning(
        "Limitation to ", min(object$ncomp),
    ...
    AvgFeatExp0$Dataset <- factor(rep(names(X), unlist(lapply(cord, nrow))),
      levels = names(X)
    )[match(AvgFeatExp0$Features, colnames(Xdat))]
    ...
    # Plot ideogram
    drawIdeogram(
      R = circleR, cir = db, W = segmentWidth, show.band.labels = TRUE,
    ...
    if (nrow(links) > 0) {
      drawLinks(
        R = linksR, cir = db, mapping = links, col = linkColors, lineWidth = linkWidth,
    ...
    if (line == TRUE) {
      for (i in 1:length(chr.names)) {
        ...
        # Reorder columns
        cOrder <- c(c(
          grep("seg.name", colnames(expr)),
          grep("seg.po", colnames(expr))
        ), c(1:length(cTypes) + 1))
        expr <- expr[, cOrder]
        ...
        drawLinePlot(
          R = linePlotR, cir = subChr, W = linePlotWidth,
    ...
    if (legend == TRUE) {
      ...
      legend(
        x = 5, y = (circleR / 4), title = "Correlations",
      ...
      if (line == TRUE) {
        legend(
          x = figSize - (circleR / 3), y = (circleR / 3), title = legend.title,
      ...
      legend(
        x = figSize - (circleR / 2), y = figSize, title = "Correlation cut-off",
        legend = paste("r", cutoff, sep = "="),
        col = "black", cex = size.legend, bty = "n"
      )

      legend(
        x = -circleR / 4, y = figSize, legend = paste(
          "Comp",
          paste(comp, collapse = "-")
        ),
        col = "black", cex = size.legend, bty = "n"
...
drawIdeogram <- function(R, xc = 400, yc = 400, cir, W,
  ...
  for (chr.i in c(1:chr.num)) {
    ...
    if (show.chr.labels) {
      ...
      if (line == TRUE) {
        draw.text.rt(xc, yc, chr.labels.R, w.m, chr.t,
          cex = size.labels,
          segmentWidth = W, parallel = TRUE
        )
      } else {
        # put the labels closer to the circle
        draw.text.rt(xc, xc, chr.labels.R, w.m, chr.t,
          cex = size.labels,
  ...
  for (chr.i in 1:chr.num) {
    ...
    for (j in col.v:ncol(dat)) {
      ...
      for (k in 1:nrow(dat)) {
        ...
        if (w.from > 0) {
          draw.line3(xc, yc, w.from, w.to, v.old, v,
            col = col,
            lwd = lineWidth
          )
        }
...
draw.text.rt <- function(xc, yc, r, w, n, col = "black", cex = 1, side = "out",
  ...
  text(x, y,
    adj = 0, offset = 1, labels = n, srt = the.w,
...
segAnglePo <- function(seg.dat = seg.dat, seg = seg, angle.start = angle.start,
  ...
  for (i in 1:seg.num) {
    ...
    out.s <- rbind(out.s, c(
      seg.n, w1 + seg.angle.from, w2 + seg.angle.from,
  ...
  colnames(out.s) <- c(
    "seg.name", "angle.start", "angle.end", "seg.sum.start",
