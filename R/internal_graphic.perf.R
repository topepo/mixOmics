...
internal_graphic.perf <-
  function(error.rate,
    ...
    if (overlay == "all") {
      out <- matplot(component, error.rate.concat,
        type = type, lty =
      ...
      if (!is.null(error.rate.sd.concat)) {
        ...
        for (j in 1:ncol(error.rate.concat)) {
          plot_error_bar(
            x = component, y = error.rate.concat[, j],
      ...
      for (di in dist)
      {
        ...
        out <- matplot(new_mat.error,
          type = type, lty = c(1:length(measure)),
        ...
        if (legend.position == "vertical") {
          legend("topright",
            legend = measure, lty = 1:length(measure),
            col = rep(color[which(di == dist)]), ncol = 1, lwd = 2
          )
        } else if (legend.position == "horizontal") {
          legend("topright",
            legend = c(measure), lty = 1:length(measure),
            col = rep(color[which(di == dist)]), ncol = 2, lwd = 2
          )
        }
        if (!is.null(error.rate.sd.concat)) {
          new_sd.error <- error.rate.sd.concat[, grep(
            di,
            colnames(error.rate.sd.concat)
          ), drop = FALSE]
          for (j in 1:ncol(new_mat.error)) {
            plot_error_bar(
              x = component, y = new_mat.error[, j],
      ...
      for (mea in measure)
      {
        new_mat.error <- error.rate.concat[, grep(
          mea,
          colnames(error.rate.concat)
        ), drop = FALSE]

        out <- matplot(new_mat.error,
          type = type, lty = c(1:length(dist)),
          col = color,
          lwd = 2, xlab = xlab, ylab = ylab, xaxt = "n", ylim = ylim
        )
        ...
        if (!is.null(error.rate.sd.concat)) {
          col <- matrix(color,
            nrow = nrow(error.rate.concat),
            ncol = length(dist), byrow = TRUE
          )

          new_sd.error <- error.rate.sd.concat[,
            grep(mea, colnames(error.rate.sd.concat)),
            drop = FALSE
          ]
          for (j in 1:ncol(new_mat.error)) {
            plot_error_bar(
...
plot_error_bar <- function(x, y = as.numeric(x), uiw, add = FALSE, col = "black") {
  ...
  arrow.args <- c(list(
    lty = par("lty"), angle = 90, length = smidge,
    code = 1, col = scols
  ))
