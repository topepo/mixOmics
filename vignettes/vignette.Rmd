...
knitr::opts_chunk$set(
  dpi = 100, echo = TRUE, warning = FALSE, message = FALSE, eval = TRUE, cache = FALSE,
...
img <- htmltools::img(
  src = knitr::image_uri("InputFigures/logo-mixomics.jpg"),
  alt = "logo",
...
```{r, eval = FALSE}
plotVar(final.spca.multi,
  comp = c(1, 2), var.names = TRUE,
  cex = 3, # To change the font size
...
Q2.pls1.liver <- perf(tune.pls1.liver,
  validation = "Mfold",
  folds = 10, nrepeat = 5
)
...
spls1.liver.c2 <- spls(X, y,
  ncomp = 2, keepX = c(rep(choice.keepX, 2)),
  mode = "regression"
)
...
legend("topleft",
  col = color.mixo(1:4), legend = levels(time.liver),
  lty = 1, title = "Time"
)
legend("bottomright",
  legend = levels(dose.liver), pch = 1:4,
  title = "Dose"
)
...
perf.pls1.liver <- perf(pls1.liver,
...
# sPLS1 performance
perf.spls1.liver <- perf(spls1.liver,
  validation = "Mfold", folds = 10,
  nrepeat = 5, progressBar = FALSE
)
...
set.seed(33) # For reproducibility with this handbook, remove otherwise
Q2.pls2.liver <- perf(tune.pls2.liver,
  validation = "Mfold", folds = 10,
  nrepeat = 5
)
...
plotVar(spls2.liver,
  var.names = list(
    X.label = liver.toxicity$gene.ID[, "geneBank"],
    Y.label = TRUE
  ), cex = c(3, 4)
)
```
...
cim(spls2.liver,
  comp = 1:2, xlab = "clinic", ylab = "genes",
...
perf.pls.liver <- perf(pls.liver,
  validation = "Mfold", folds = 10,
  nrepeat = 5
)

## Performance for the sPLS model ran earlier
perf.spls.liver <- perf(spls2.liver,
  validation = "Mfold", folds = 10,
  nrepeat = 5
)
```
...
set.seed(30) # For reproducibility with this handbook, remove otherwise
perf.plsda.srbct <- perf(plsda.srbct,
...
```{r 05-plsda-perf}
set.seed(30) # For reproducibility with this handbook, remove otherwise
perf.final.plsda.srbct <- perf(final.plsda.srbct,
  validation = "Mfold",
  folds = 3,
...
perf.splsda.srbct <- perf(splsda.srbct,
...
test.keepX <- list(
  mRNA = c(5:9, seq(10, 25, 5)),
  miRNA = c(5:9, seq(10, 20, 2)),
  proteomics = c(seq(5, 25, 5))
)

tune.diablo.tcga <- tune.block.splsda(X, Y,
  ncomp = 2,
...
perf.diablo.tcga <- perf(diablo.tcga,
  validation = "Mfold", folds = 10,
...
auc.diablo.tcga <- auroc(diablo.tcga,
  roc.block = "miRNA", roc.comp = 2,
  print = FALSE
)
```
...
```{r}
confusion.mat.tcga <- get.confusion_matrix(
...
plotIndiv(final.mint.plsda.stem,
  legend = TRUE, title = "MINT PLS-DA",
...
```{r 07-splsda-final}
final.mint.splsda.stem <- mint.splsda(
...
plotIndiv(final.mint.splsda.stem,
  study = "global", legend = TRUE,
...
cim(final.mint.splsda.stem,
  comp = 1, margins = c(10, 5),
...
# If facing margin issues, use either X11() or save the plot using the
# arguments save and name.save
network(final.mint.splsda.stem,
  comp = 1,
...
plotLoadings(final.mint.splsda.stem,
...
mint.predict.stem <- predict(mint.splsda.stem2,
  newdata = X[indiv.test, ],
  dist = "centroids.dist",
